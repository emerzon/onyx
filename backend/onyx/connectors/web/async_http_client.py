...
class AsyncHttpClient:
    ...
    def __init__(
        ...
        # Default headers
        self.headers = {"User-Agent": "Onyx-WebConnector/2.0"}
    ...
    async def _make_request_with_retry(
        self, method: str, url: str, headers: Optional[Dict[str, str]] = None, timeout: Optional[float] = None, **kwargs
    ) -> httpx.Response:
        """
        ...
        for attempt in range(self.max_retries + 1):
            try:
                response = await self._client.request(
                    method=method, url=url, headers=request_headers, timeout=request_timeout, **kwargs
                )
                ...
                if e.response.status_code >= 500 and attempt < self.max_retries:
                    backoff_time = self.retry_backoff_factor * (2**attempt)
                    logger.warning(
                        f"Server error {e.response.status_code} on attempt {attempt + 1} for {url}, retrying in {backoff_time:.1f}s"
                    )
                    await asyncio.sleep(backoff_time)
                    continue
                else:
    ...
    async def stream_content(
        ...
        async with self._client.stream("GET", url, headers=request_headers, timeout=request_timeout) as response:
            response.raise_for_status()
