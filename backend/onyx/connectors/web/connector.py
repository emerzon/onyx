...
WebConnector = AsyncWebConnectorCrawlee
...
Examples:
  ...
  python connector.py --url https://docs.example.com --type recursive --save-batches --batch-dir ./ingestion_batches
    ...
    parser.add_argument(
        "--type", "-t", choices=["single", "recursive", "sitemap"], default="single", help="Crawling type (default: single)"
    )
    ...
    parser.add_argument("--no-mintlify", action="store_true", help="Disable Mintlify-specific cleanup")

    parser.add_argument("--no-specialized", action="store_true", help="Disable specialized extractors (Zoomin, etc.)")

    parser.add_argument("--verbose", "-v", action="store_true", help="Enable verbose logging")

    parser.add_argument("--output", "-o", help="Output file to save extracted content (JSON format)")

    parser.add_argument("--save-batches", action="store_true", help="Save each batch as a separate JSON file for ingestion API")

    parser.add_argument("--batch-dir", default="./batches", help="Directory to save batch files (default: ./batches)")
    ...
    parser.add_argument("--skip-images", action="store_true", help="Skip image files (PNG, JPG, GIF, etc.) during crawling")
    ...
    def save_batch_for_ingestion(batch_docs: list, batch_num: int, source_url: str) -> str:
        ...
        for doc in batch_docs:
            ...
            ingestion_doc = {
                "document": {
                    "id": doc.id,
                    "sections": [{"text": text_content, "link": source_link or doc.id}],
                    "source": "web",
